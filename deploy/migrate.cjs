...
const MIGRATIONS = [
  ...
  `ALTER TABLE branches ADD COLUMN IF NOT EXISTS created_at TIMESTAMP NOT NULL DEFAULT NOW()`,
  `ALTER TABLE branches ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP NOT NULL DEFAULT NOW()`,
  `CREATE UNIQUE INDEX IF NOT EXISTS branches_user_code_idx ON branches (user_id, lower(code))`,
