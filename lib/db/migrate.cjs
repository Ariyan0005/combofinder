...
const MIGRATIONS = [
  ...
  // ── Compatibility Database (unified: Category -> Model -> Compatible Models) ──
  // Preserve any old part_categories data by promoting it to the new table name.
  `ALTER TABLE IF EXISTS part_categories RENAME TO compatibility_categories`,
  ...
  `CREATE TABLE IF NOT EXISTS compatibility_links (
    id SERIAL PRIMARY KEY,
    category_id INTEGER NOT NULL REFERENCES compatibility_categories(id) ON DELETE CASCADE,
    model_id INTEGER NOT NULL REFERENCES models(id) ON DELETE CASCADE,
    compatible_model_id INTEGER NOT NULL REFERENCES models(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
    UNIQUE(category_id, model_id, compatible_model_id)
  )`,
  // Old "parts" table used an inconsistent shape (no compatible-models link).
  // Keep its data around under a deprecated name instead of deleting it outright.
  `ALTER TABLE IF EXISTS parts RENAME TO _deprecated_parts`,
];
