...
const MIGRATIONS = [
  ...
  // 1h. Copy data from combos → compatibilities if combos table has rows and compatibilities is empty
  //     (handles case where previous migration rename failed due to syntax error)
  `DO $
  DECLARE
  ...
  BEGIN
    ...
    IF combos_exists AND compat_empty THEN
      INSERT INTO compatibilities (model_id, name, combo_type, quality_grade, notes, created_at)
      SELECT
        model_id,
        name,
      ...
      FROM combos;
      RAISE NOTICE 'Copied % rows from combos to compatibilities', (SELECT COUNT(*) FROM compatibilities);
    END IF;
  END $`,
