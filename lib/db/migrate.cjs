...
const MIGRATIONS = [
  ...
  // 1g. Rename combos table → compatibilities (only if combos exists and compatibilities doesn't)
  `DO $ BEGIN
    IF EXISTS (
  ...
  // 1h. Create compatibilities table if it still doesn't exist (fresh DB — no combos to rename)
