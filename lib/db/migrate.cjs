...
const MIGRATIONS = [
  ...
  // are preserved; this only makes the legacy constraint compatible with the
  ...
  // Existing users keep the mobile-repair experience unless they explicitly
  // selected General Store during registration.
  ...
  // Wrapped in DO block — parts table may not exist on all deployments
  `DO $
  BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'parts') THEN
      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'parts' AND column_name = 'category_id') THEN
        ALTER TABLE parts ADD COLUMN category_id INTEGER REFERENCES part_categories(id) ON DELETE CASCADE;
      END IF;
      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'parts' AND column_name = 'model_id') THEN
        ALTER TABLE parts ADD COLUMN model_id INTEGER REFERENCES models(id) ON DELETE CASCADE;
      END IF;
    END IF;
  END $`,
  ...
  `ALTER TABLE compatibilities ADD COLUMN IF NOT EXISTS part_type TEXT`,
  `ALTER TABLE compatibilities ADD COLUMN IF NOT EXISTS image_url TEXT`,
  `ALTER TABLE compatibilities ADD COLUMN IF NOT EXISTS is_published BOOLEAN NOT NULL DEFAULT FALSE`,
  `CREATE INDEX IF NOT EXISTS compatibilities_model_id_idx ON compatibilities (model_id)`,
  `CREATE INDEX IF NOT EXISTS compatibilities_published_idx ON compatibilities (is_published)`,
  `CREATE INDEX IF NOT EXISTS compatibilities_part_type_idx ON compatibilities (part_type)`,

  // ── Publishable ISP and pinout records ─────────────────────────────────
  `CREATE TABLE IF NOT EXISTS schematics (
    ...
    device_brand TEXT,
    device_model TEXT,
    schematic_type TEXT DEFAULT 'Motherboard',
    file_url TEXT,
    thumbnail_url TEXT,
    file_size TEXT,
    tags TEXT,
    ...
    ground TEXT,
    signal_info TEXT,
    test_point_info TEXT,
    is_published BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
  )`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS slug TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS model_id INTEGER REFERENCES models(id) ON DELETE SET NULL`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS notes TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS component TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS pin_number TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS pin_name TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS voltage TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS ground TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS signal_info TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS test_point_info TEXT`,
  `ALTER TABLE schematics ADD COLUMN IF NOT EXISTS is_published BOOLEAN NOT NULL DEFAULT FALSE`,
  `CREATE INDEX IF NOT EXISTS schematics_slug_idx ON schematics (slug)`,
  `CREATE INDEX IF NOT EXISTS schematics_model_id_idx ON schematics (model_id)`,
  `CREATE INDEX IF NOT EXISTS schematics_published_idx ON schematics (is_published)`,
  `CREATE INDEX IF NOT EXISTS schematics_type_idx ON schematics (schematic_type)`,
  ...
  // Add invoice_number to group multiple items under one invoice
  `ALTER TABLE supplier_purchases ADD COLUMN IF NOT EXISTS invoice_number TEXT`,
  ...
  // ── Seed remaining Compatibility categories ───────────────────────────
  `INSERT INTO categories (name, slug, sort_order)
   VALUES ('Battery', 'battery', 1)
   ON CONFLICT (slug) DO NOTHING`,
  ...
  `INSERT INTO categories (name, slug, sort_order)
   VALUES ('ISP & Pinout', 'isp-pinout', 3)
   ON CONFLICT (slug) DO NOTHING`,

  // ── Battery Models (battery part numbers per brand) ───────────────────
  ...
  // ── Battery Compatibility (devices per battery model) ─────────────────
  `CREATE TABLE IF NOT EXISTS battery_compatibility (
    id SERIAL PRIMARY KEY,
    battery_model_id INTEGER NOT NULL REFERENCES battery_models(id) ON DELETE CASCADE,
    device_name TEXT NOT NULL,
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
  )`,

  // ── IC Models (IC chip numbers per brand) ─────────────────────────────
  `CREATE TABLE IF NOT EXISTS ic_models (
    id SERIAL PRIMARY KEY,
    brand_id INTEGER NOT NULL REFERENCES brands(id) ON DELETE CASCADE,
  ...
  // ── IC Compatibility (devices per IC model) ───────────────────────────
  `CREATE TABLE IF NOT EXISTS ic_compatibility (
    id SERIAL PRIMARY KEY,
    ic_model_id INTEGER NOT NULL REFERENCES ic_models(id) ON DELETE CASCADE,
    device_name TEXT NOT NULL,
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
  )`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_parts_suppliers_country ON parts_suppliers(country)`,
  `CREATE INDEX IF NOT EXISTS idx_parts_suppliers_active ON parts_suppliers(is_active)`,

  // ── Supplier Reviews ───────────────────────────────────────────────────
  `CREATE TABLE IF NOT EXISTS supplier_reviews (
    id SERIAL PRIMARY KEY,
    supplier_id INTEGER NOT NULL REFERENCES parts_suppliers(id) ON DELETE CASCADE,
    user_id INTEGER,
  ...
  `ALTER TABLE staff ADD COLUMN IF NOT EXISTS branch_name TEXT`,
  `CREATE UNIQUE INDEX IF NOT EXISTS staff_owner_username_idx ON staff (user_id, username) WHERE username IS NOT NULL`,
...
async function run() {
  ...
  try {
    ...
    // The migration loop intentionally tolerates legacy-schema differences,
    // but registration must not run unless its required schema is present.
