...
// ── Types ─────────────────────────────────────────────────────────────────────
type Pinout   = { id: number; title: string; slug?: string | null; deviceBrand?: string; deviceModel?: string; fileUrl?: string; thumbnailUrl?: string; schematicType?: string; tags?: string; isPublished?: boolean };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)" }}>
      <div className="bg-card border border-border rounded-2xl w-full max-w-md p-6 space-y-4 shadow-2xl">
        ...
        <label className="flex items-center gap-2 text-sm">
          ...
          Publish this record on the public ISP page
        </label>
...
export default function IspPinoutAdmin() {
  ...
  async function togglePublish(pin: Pinout) {
    const res = await fetch(`/api/schematics/${pin.id}`, {
      method: "PUT",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ isPublished: !pin.isPublished }),
    });
    if (res.ok) qc.invalidateQueries({ queryKey: ["admin-isp-pinouts"] });
  }
                    ...
                    <div key={pin.id} className="rounded-xl border border-border overflow-hidden bg-card relative group">
                      ...
                      <div className="p-2.5 space-y-1">
                        ...
                        {pin.deviceModel && (
                         ...
                         <p className={`text-[10px] font-semibold ${pin.isPublished ? "text-emerald-600" : "text-muted-foreground"}`}>
                           {pin.isPublished ? "Published" : "Draft"}
                         </p>
                        <div className="flex items-center gap-1.5 pt-0.5">
                           {pin.isPublished && pin.slug && (
                             <a href={`/isp/${pin.slug}`} target="_blank" rel="noreferrer"
                          ...
                          {pin.thumbnailUrl && (
                           ...
                           <button onClick={() => togglePublish(pin)}
                             className="text-[10px] font-semibold text-primary hover:underline">
                             {pin.isPublished ? "Unpublish" : "Publish"}
                           </button>
