...
interface BatteryModel {
  ...
  modelNumber: string; capacity?: string; voltage?: string;
}
interface DeviceCompat {
  id: number; batteryModelId: number; deviceName: string; notes?: string; createdAt: string;
}

export default function BatteryCompat() {
  const { id } = useParams<{ id: string }>();
  const batteryModelId = Number(id);
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["battery-compat", batteryModelId] });

  const addMut = useMutation({
    mutationFn: () =>
      fetch("/api/battery-compat", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ batteryModelId, deviceName: singleDevice.trim(), notes: singleNotes.trim() || null }),
      }).then(r => r.json()),
  ...
  const bulkMut = useMutation({
    mutationFn: (deviceNames: string[]) =>
      fetch("/api/battery-compat/bulk", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ batteryModelId, deviceNames }),
      }).then(r => r.json()),
    onSuccess: (data) => {
  ...
  const deleteMut = useMutation({
    mutationFn: (id: number) =>
      fetch(`/api/battery-compat/${id}`, { method: "DELETE", credentials: "include" }).then(r => r.json()),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link href={`/battery-brands/${batteryModel?.brandId}/models`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            ...
            {!batteryModel?.capacity && !batteryModel?.voltage && "Devices compatible with this battery"}
          </p>
        </div>
      ...
      <Dialog open={isBulkOpen} onOpenChange={o => !o && setIsBulkOpen(false)}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button size="sm" onClick={handleBulkAdd} disabled={!bulkText.trim() || bulkMut.isPending}>
