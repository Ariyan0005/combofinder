...
import { Plus, Edit2, Trash2, Cpu, Search, Layers, ChevronRight } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog";
import { Label } from "@/components/ui/label";

const ISP_CATEGORY_SLUG = "isp";
...
export default function IspBrands() {
  ...
  const ispCategory = categories.find((c) => c.slug === ISP_CATEGORY_SLUG);
  const categoryId = ispCategory?.id;
  ...
  const handleCreate = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!categoryId) {
      toast({ title: "ISP category is not set up yet. Please create an 'isp' category first.", variant: "destructive" });
      return;
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">ISP & Pinout — Brands</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Manage device brands used in ISP pinout diagrams.</p>
          {!categoryId && !categoriesLoading && (
            <p className="text-xs text-destructive mt-1">
              ⚠ No "isp" category found. Go to Settings → Categories and create one with slug <code className="bg-muted px-1 rounded">isp</code>.
            </p>
          )}
        </div>
              ...
              filteredBrands.map((brand) => (
                <TableRow key={brand.id} className="group hover:bg-muted/30 transition-colors">
                  <TableCell className="font-medium">
                    <Link href={`/isp-brands/${brand.id}/models`}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setEditingBrand(brand)}>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="max-w-sm">
          <form onSubmit={handleCreate}>
            <DialogHeader>
              <DialogTitle>Add ISP Brand</DialogTitle>
            </DialogHeader>
