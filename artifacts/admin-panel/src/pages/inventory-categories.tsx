...
const API = "/api/inventory-categories";
...
export default function InventoryCategories() {
  ...
  const createM = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["inventory-categories"] }); setIsCreateOpen(false); toast({ title: "Category added" }); },
    onError: () => toast({ title: "Failed to add category", variant: "destructive" }),
  });

  const updateM = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<InventoryCategory> }) => {
      const r = await fetch(`${API}/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data), credentials: "include" });
      if (!r.ok) throw new Error("Failed");
      return r.json();
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["inventory-categories"] }); setEditingCat(null); toast({ title: "Category updated" }); },
  ...
  const deleteM = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["inventory-categories"] }); toast({ title: "Category deleted" }); },
  ...
  const getFormData = (form: HTMLFormElement) => ({
    ...
    parentId: (form.elements.namedItem("parentId") as HTMLInputElement).value ? Number((form.elements.namedItem("parentId") as HTMLInputElement).value) : undefined,
  });

  function CategoryForm({ def }: { def?: InventoryCategory }) {
    return (
      <div className="space-y-4 py-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1"><Label className="text-xs">Icon Name</Label><Input name="icon" defaultValue={def?.icon} placeholder="e.g. Package" /></div>
        ...
        <div className="space-y-1"><Label className="text-xs">Parent Category ID</Label><Input name="parentId" type="number" defaultValue={def?.parentId} placeholder="Optional parent ID" /></div>
    ...
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Manage categories for parts and stock.</p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/40 hover:bg-muted/40">
              ...
              <TableHead className="font-semibold text-xs uppercase tracking-wider hidden md:table-cell">Parent</TableHead>
              ...
              categories.map(c => (
                <TableRow key={c.id} className="group hover:bg-muted/30 transition-colors">
                  ...
                  <TableCell className="text-xs text-muted-foreground hidden md:table-cell">
                    {c.parentId ? `Parent ID: ${c.parentId}` : "—"}
                  </TableCell>
