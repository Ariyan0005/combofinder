...
import { Plus, Edit2, Trash2, MemoryStick, Search, Layers, ChevronRight } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog";
import { Label } from "@/components/ui/label";

const IC_CATEGORY_SLUG = "ic";
...
export default function IcBrands() {
  const { toast } = useToast();

  // Resolve the IC category id (created once, reused across brand CRUD calls).
  ...
  const icCategory = categories.find((c) => c.slug === IC_CATEGORY_SLUG);
  const categoryId = icCategory?.id;
  ...
  const handleCreate = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!categoryId) {
      toast({ title: "IC category is not set up yet", variant: "destructive" });
      return;
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">IC Compatibility</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Manage IC brands and model compatibility.</p>
        </div>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="max-w-sm">
          <form onSubmit={handleCreate}>
            <DialogHeader>
              <DialogTitle>Add New IC Brand</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-1.5">
                <Label htmlFor="name">Brand Name *</Label>
                <Input id="name" name="name" required autoFocus placeholder="e.g. Qualcomm" />
              </div>
              <div className="space-y-1.5">
