...
import { Plus, Search, Edit2, Trash2, FolderOpen, Download, Tag, ExternalLink } from "lucide-react";
...
export default function Schematics() {
  ...
  const getFormData = (form: HTMLFormElement) => ({
    ...
    component: (form.elements.namedItem("component") as HTMLInputElement).value.trim() || undefined,
    pinNumber: (form.elements.namedItem("pinNumber") as HTMLInputElement).value.trim() || undefined,
    pinName: (form.elements.namedItem("pinName") as HTMLInputElement).value.trim() || undefined,
    voltage: (form.elements.namedItem("voltage") as HTMLInputElement).value.trim() || undefined,
    ground: (form.elements.namedItem("ground") as HTMLInputElement).value.trim() || undefined,
    signalInfo: (form.elements.namedItem("signalInfo") as HTMLInputElement).value.trim() || undefined,
    testPointInfo: (form.elements.namedItem("testPointInfo") as HTMLInputElement).value.trim() || undefined,
  ...
  function SchematicForm({ def }: { def?: Schematic }) {
    return (
      <div className="space-y-4 py-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1"><Label className="text-xs">Type *</Label>
            <select name="schematicType" defaultValue={def?.schematicType || "Motherboard"} className="flex h-9 w-full rounded-md border border-input bg-card px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
              ...
              <option value="ISP Pinout">ISP Pinout</option>
        ...
        <div className="space-y-1"><Label className="text-xs">Technical notes</Label><Textarea name="notes" defaultValue={def?.notes ?? ""} rows={3} placeholder="Usage notes, warnings, or repair guidance" /></div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1"><Label className="text-xs">Component</Label><Input name="component" defaultValue={def?.component ?? ""} placeholder="eMMC, LCD…" /></div>
          <div className="space-y-1"><Label className="text-xs">Pin number</Label><Input name="pinNumber" defaultValue={def?.pinNumber ?? ""} placeholder="TP1 / 5" /></div>
          <div className="space-y-1"><Label className="text-xs">Pin name / function</Label><Input name="pinName" defaultValue={def?.pinName ?? ""} placeholder="CLK, VCC…" /></div>
          <div className="space-y-1"><Label className="text-xs">Voltage</Label><Input name="voltage" defaultValue={def?.voltage ?? ""} placeholder="1.8 V" /></div>
          <div className="space-y-1"><Label className="text-xs">Ground</Label><Input name="ground" defaultValue={def?.ground ?? ""} placeholder="GND / pin 2" /></div>
          <div className="space-y-1"><Label className="text-xs">Signal information</Label><Input name="signalInfo" defaultValue={def?.signalInfo ?? ""} placeholder="UART TX" /></div>
          <div className="space-y-1 col-span-2"><Label className="text-xs">Test point</Label><Input name="testPointInfo" defaultValue={def?.testPointInfo ?? ""} placeholder="TP location or probing guidance" /></div>
        </div>
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" name="isPublished" defaultChecked={def?.isPublished === true} className="h-4 w-4" />
          Publish this record on the public technical page
        </label>
      </div>
    );
  }
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/40 hover:bg-muted/40">
              ...
              <TableHead className="font-semibold text-xs uppercase tracking-wider">Public</TableHead>
                  ...
                  <TableCell className="text-xs">
                    {s.isPublished ? (
                      <span className="inline-flex items-center gap-2">
                        <a href={`/${s.schematicType === "ISP Pinout" ? "isp" : "pinout"}/${s.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-semibold text-emerald-600 hover:underline">
                          Published <ExternalLink className="h-3 w-3" />
                        </a>
                        <button className="text-muted-foreground hover:text-foreground hover:underline" onClick={() => updateM.mutate({ id: s.id, data: { isPublished: false } })}>Unpublish</button>
                      </span>
                    ) : (
                      <button className="font-semibold text-primary hover:underline" onClick={() => updateM.mutate({ id: s.id, data: { isPublished: true } })}>Publish</button>
                    )}
                  </TableCell>
