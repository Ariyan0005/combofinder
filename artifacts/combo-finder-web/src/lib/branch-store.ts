  ...
  try {
    ...
    const customFetch = (input: RequestInfo | URL, init?: RequestInit) => {
      try {
        ...
        if (url.startsWith("/api/") || url.includes("/api/")) {
          ...
          Object.entries(getActiveBranchHeaders()).forEach(([key, value]) => {
    ...
    try {
      ...
      try {
        ...
        // Ignore if read-only window.fetch
      }
    }
  } catch {
...
export function useBranchSelection() {
  ...
  return { branches, activeBranch, activeId, branchParam, selectBranch, refresh, isLoading };
}
