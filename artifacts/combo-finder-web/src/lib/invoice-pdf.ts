...
async function buildInvoiceDoc(invoice: InvoiceData): Promise<jsPDF> {
  ...
  const phone = (invoice.shopPhone ?? "").trim();
  const shopContact = [address, phone ? `Phone: ${phone}` : ""].filter(Boolean).join("  |  ");
  const hasContact = shopContact.length > 0;
  const headerH = hasContact ? 36 : 32;
  ...
  if (hasContact) {
    ...
    const addrLines = doc.splitTextToSize(shopContact, W - 14 - logoAreaW - 20);
...
function buildRepairDoc(r: RepairVoucherData): jsPDF {
  ...
  const address = (r.shopAddress ?? "").trim();
  const phone = (r.shopPhone ?? "").trim();
  const shopContact = [address, phone ? `Phone: ${phone}` : ""].filter(Boolean).join("  |  ");
  const headerH = shopContact ? 32 : 28;
  ...
  if (shopContact) {
    ...
    doc.text(shopContact, L, 16);
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8);
    doc.text("Repair Voucher", L, 21);
  } else {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
  ...
  doc.roundedRect(L, headerH - 7, stW, 5.5, 1, 1, "F");
  doc.setTextColor(hr, hg, hb);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(6.5);
  doc.text(stText, L + stW / 2, headerH - 3, { align: "center" });
  doc.setTextColor(0, 0, 0);
