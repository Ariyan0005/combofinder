...
export const localLedger = {
  ...
  getAccount(uid: number, accountId: number): any | null {
    ...
    return { ...acc, creditSum, debitSum, balance: debitSum - creditSum, entries };
  },
...
export const localSales = {
  ...
  applyPayment(uid: number, amount: number, saleId?: number | number[]): { applied: number; updated: any[] } {
    const items = read<any>(sKey(uid));
    let remaining = Math.max(0, Number(amount) || 0);
    const saleIdArray = Array.isArray(saleId) ? saleId : (saleId !== undefined ? [saleId] : null);
    const candidates = items
      .map((sale: any, index: number) => ({ sale, index }))
      .filter(({ sale }) =>
        sale.paymentMethod === "Credit" &&
        sale.status !== "Returned" &&
        (!saleIdArray || saleIdArray.includes(sale.id)),
      )
      .sort((a, b) => {
        const createdAtOrder = String(a.sale.createdAt ?? "").localeCompare(String(b.sale.createdAt ?? ""));
        return createdAtOrder || b.index - a.index;
      });
    const updated: any[] = [];

    for (const { sale, index } of candidates) {
      if (remaining <= 0) break;
      const due = Math.max(
        0,
        Number(sale.total ?? 0) -
          Number(sale.advancePaid ?? 0) -
          Number(sale.totalRefund ?? 0),
      );
      if (due <= 0) continue;
      const applied = Math.min(due, remaining);
      items[index] = {
        ...sale,
        advancePaid: Number(sale.advancePaid ?? 0) + applied,
      };
      remaining -= applied;
    ...
    if (applied > 0) write(sKey(uid), items);
    return { applied, updated };
  },
  returnItems(uid: number, saleId: number, returnItemsList: { saleItemId?: number; partName?: string; quantity: number }[], reason?: string): any {
    const items = read<any>(sKey(uid));
    const idx = items.findIndex((i: any) => i.id === saleId);
    if (idx === -1) throw new Error("Sale not found");
    const sale = items[idx];
    const saleItems = (sale.items || []).map((it: any) => ({ ...it }));
    let refundTotal = 0;
    const now = new Date();
    const dateStr = localDateStr(now);
    const returns = [...(sale.returns || [])];

    for (const ret of returnItemsList) {
      const sItem = saleItems.find((it: any) => (ret.saleItemId ? it.id === ret.saleItemId : it.partName === ret.partName));
      if (sItem) {
        const q = Math.min(ret.quantity, sItem.quantity - (sItem.returnedQuantity || 0));
        if (q > 0) {
          sItem.returnedQuantity = (sItem.returnedQuantity || 0) + q;
          const refundAmt = Number(sItem.unitPrice || 0) * q;
          ...
          returns.push({
            ...
            reason: reason || "Returned bill",
            date: dateStr,
          });

          // Also restore local inventory quantity if inventoryId is present
          if (sItem.inventoryId) {
            try {
              const invList = read<any>(iKey(uid));
              const invIdx = invList.findIndex((inv: any) => inv.id === sItem.inventoryId);
              if (invIdx !== -1) {
                invList[invIdx].quantity = (Number(invList[invIdx].quantity) || 0) + q;
                write(iKey(uid), invList);
              }
            } catch {}
          }
        }
    ...
    const fullyReturned = saleItems.every((it: any) => (it.returnedQuantity || 0) >= it.quantity);
    const anyReturned = saleItems.some((it: any) => (it.returnedQuantity || 0) > 0);
    const newStatus = fullyReturned ? "Returned" : (anyReturned ? "Partially Returned" : (sale.status || "Completed"));

    items[idx] = {
      ...sale,
      status: newStatus,
      returns,
      totalRefund: (Number(sale.totalRefund) || 0) + refundTotal,
      items: saleItems,
    };
    write(sKey(uid), items);
    return items[idx];
...
type LocalCategory = {
  ...
  parentId?: number | null; icon?: string | null; color?: string | null;
  branchId?: string | number | null; branchName?: string | null; createdAt: string;
};

export const localCategories = {
  getAll(uid: number, branchParam?: string): LocalCategory[] {
    const all = read<LocalCategory>(catKey(uid));
    if (!branchParam || branchParam === "all") return all;
    if (branchParam === "main" || branchParam === "default-main" || branchParam === "default" || branchParam === "0") {
      return all.filter(c => !c.branchId || c.branchId === "default" || c.branchId === "main" || c.branchId === "default-main" || Number(c.branchId) === 0);
    }
    return all.filter(c => String(c.branchId) === String(branchParam));
  },
