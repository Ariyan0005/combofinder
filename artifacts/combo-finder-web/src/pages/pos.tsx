...
function PosMain() {
  ...
  const { data: customers = [] } = useQuery<Customer[]>({
    queryKey: ["customers", isPro, user?.id, activeBranch?.id],
    queryFn: () => {
      if (!isPro && user?.id) return Promise.resolve(localCustomers.getAll(user.id) as Customer[]);
      return fetch(`/api/customers?branchId=${encodeURIComponent(branchParam)}`, { credentials: "include" }).then(r => r.json());
    },
    enabled: !!user?.id,
  });

  const { data: staffData = [] } = useQuery<any[]>({
    queryKey: ["staff", isPro, user?.id, activeBranch?.id, branchParam],
    queryFn: async () => {
  ...
  const staffList = (Array.isArray(staffData) ? staffData : []).filter((s: any) =>
    s.isActive && (s.role === "Staff" || s.role === "Manager")
  ).length > 0
    ? (Array.isArray(staffData) ? staffData : []).filter((s: any) => s.isActive && (s.role === "Staff" || s.role === "Manager"))
