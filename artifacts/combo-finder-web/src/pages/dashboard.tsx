...
import {
  ...
  ChevronDown, Building2, CheckCircle2, ChevronRight,
} from "lucide-react";
...
const WHATSAPP_URL     = "https://wa.me/96897043234?text=Hi%21+I+need+support.+I%27m+contacting+you+from+the+PosCert+app.";
...
function getLocalTimeStr() {
  const now = new Date();
  return now.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric", year: "numeric" }) + " • " + now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
}
...
// ── Quick Actions ─────────────────────────────────────────────────────────────
const QUICK_LINKS = [
  { label: "Party's",      icon: Users,        href: "/customers",     color: "#8B5CF6", bg: "#F5F3FF" },
...
export default function Dashboard() {
  const { user } = useAuth();
  // Only the owner can switch branches. Staff and managers stay scoped to
  // the branch assigned to their login.
  const isPaidOwner = Boolean(user && !user.isStaff && !user.isManager && user.plan && ["pro", "premium"].some(plan => user.plan!.toLowerCase().startsWith(plan)));
  const { branches, activeBranch, selectBranch } = useBranchSelection();
  ...
  const isOwner = Boolean(user && !isStaff && !isManager);
  const useLocalSummary = isFreePlan && isOwner;
  ...
  const { data: stats } = useQuery<{ lowStock?: number }>({
    queryKey: ["stats", activeBranch?.id],
    queryFn:  () => fetch(`/api/stats?branchId=${encodeURIComponent(activeBranch?.code === "MAIN" ? "MAIN" : (activeBranch?.id || ""))}`, { credentials: "include" }).then(r => r.json()),
    enabled:  !useLocalSummary,
  });

  // ── Sales Summary — Pro: API, Free: localStorage ───────────────────────────
  const localSummary = useMemo(() => {
    if (!useLocalSummary || !user?.id) return null;
    return computeSalesSummary(user.id, "month");
  }, [useLocalSummary, user?.id]);

  const { data: apiSummary } = useQuery<any>({
    queryKey: ["sales-summary", "month", activeBranch?.id],
    queryFn:  () => fetch(`/api/sales-summary?range=month&tz=${encodeURIComponent(Intl.DateTimeFormat().resolvedOptions().timeZone)}&branchId=${encodeURIComponent(activeBranch?.code === "MAIN" ? "MAIN" : (activeBranch?.id || ""))}`, { credentials: "include" }).then(r => r.json()),
    enabled:  !useLocalSummary,
  });

  const summary = useLocalSummary ? localSummary : apiSummary;
  ...
  // ── Technician Tool visibility toggle (Active / Inactive) ────────────────────
  const [showTechTools, setShowTechTools] = useState<boolean>(() => {
    try {
      const saved = localStorage.getItem("poscert_show_tech_tools");
  ...
  const toggleTechTools = () => {
    setShowTechTools(prev => {
      const next = !prev;
      try {
        localStorage.setItem("poscert_show_tech_tools", String(next));
      } catch {}
      return next;
    });
  };
  ...
  const KPI_CARDS = user?.isStaff ? [
    { label: "Revenue", value: totalRevenue, color: "#10B981", bg: "#ECFDF5", prefix: sym },
    { label: "Expenses", value: 0, color: "#F97316", bg: "#FFF7ED", prefix: sym },
    { label: "Net Profit", value: 0, color: PRIMARY, bg: "hsl(var(--primary) / 0.1)", prefix: sym },
      ...
      <div className="space-y-5 pb-6">
              ...
              <div className="relative" ref={branchMenuRef}>
                ...
                >
                  <h1 className="text-lg font-extrabold leading-tight mt-0.5 flex items-center gap-2">
                    {user?.shopName || user?.name || "My Business"}
                    <span className="w-2 h-2 rounded-full bg-emerald-400 ring-2 ring-emerald-400/30 inline-block" />
                    <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform duration-200 ${branchMenuOpen ? "rotate-180 text-primary" : ""}`} />
                  </h1>
                </button>
                <p className="mt-1 flex items-center gap-1 text-[11px] font-semibold text-primary">
                  <Building2 className="h-3 w-3" /> {activeBranch?.name || "Default Branch"}
                </p>
                {branchMenuOpen && (
                  <div className="absolute left-0 top-14 z-30 w-72 rounded-2xl border border-border bg-card p-2 shadow-2xl animate-in fade-in zoom-in-95 duration-100">
                    ...
                    <div className="space-y-1">
                      {branches.filter((branch) => branch.status === "Active").map((branch) => {
                        const isSelected = branch.id === activeBranch?.id || (branch.code === "MAIN" && activeBranch?.code === "MAIN");
                        return (
                          <button
                            key={branch.id}
                            type="button"
                          ...
                          >
                            <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${isSelected ? "bg-primary text-white" : "bg-primary/10 text-primary"}`}>
                              <Building2 className="h-4 w-4" />
                            </span>
                            <span className="min-w-0 flex-1">
                              <span className="block truncate text-xs font-bold">{branch.name}</span>
                              <span className="block text-[10px] text-muted-foreground">{branch.city} · {branch.code}</span>
                            </span>
                    ...
                    <Link href="/branches">
                      <span
                        ...
                        className="mt-1.5 block border-t border-border px-3 py-2.5 text-xs font-bold text-primary hover:underline cursor-pointer"
                      >
                        Manage branches →
                      </span>
                    </Link>
        ...
        <div>
          ...
          >
            ...
            <div style={{ flex: "0 0 100%", height: HERO_SLIDE_HEIGHT, scrollSnapAlign: "start" }}>
              <div className="rounded-2xl p-4 space-y-3"
                style={{ background: "linear-gradient(135deg, hsl(var(--primary)) 0%, hsl(262 80% 55%) 100%)", height: HERO_SLIDE_HEIGHT, minHeight: HERO_SLIDE_HEIGHT, maxHeight: HERO_SLIDE_HEIGHT, boxSizing: "border-box", overflow: "hidden" }}>
            ...
            {/* Owners can swipe to the monthly KPI summary. Staff stay on the sales slide. */}
            {!isStaff && <div style={{ flex: "0 0 100%", height: HERO_SLIDE_HEIGHT, scrollSnapAlign: "start" }}>
              <div className="rounded-2xl p-4 space-y-3"
                style={{ background: "linear-gradient(135deg, hsl(var(--primary)) 0%, hsl(262 80% 55%) 100%)", height: HERO_SLIDE_HEIGHT, minHeight: HERO_SLIDE_HEIGHT, maxHeight: HERO_SLIDE_HEIGHT, boxSizing: "border-box", overflow: "hidden" }}>
        ...
        {(stats?.lowStock ?? 0) > 0 && (
          <Link href="/inventory?filter=low">
            <div className="flex items-center gap-3 px-4 py-3 rounded-2xl cursor-pointer hover:opacity-95 transition-opacity"
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {QUICK_LINKS.map(({ label, icon: Icon, href, color, bg }) => (
              <Link key={href + label} href={href}>
                <button className="w-full flex flex-col items-center gap-2 p-3.5 rounded-2xl border border-border bg-card hover:border-primary/40 transition">
        ...
        {/* ── Technician Tool — mobile-repair users only ── */}
              ...
              <div className="flex items-center justify-between relative z-10">
                <div>
                  ...
                  <p className="text-[11px] text-white/80 leading-snug">
                    {user?.businessType === "general_store" ? "Unlimited inventory & Cloud backup" : "Only $7/month · Unlimited repairs & inventory"}
                  </p>
                </div>
