      ...
      <div className="relative w-full max-w-md rounded-t-3xl md:rounded-3xl shadow-2xl p-5" style={{ background: CARD }}>
        ...
        <form onSubmit={handleAddCustomer}
          ...
          {[
            ...
            { label: "Phone", key: "phone", placeholder: "+1 234 567 8900", type: "tel" },
            { label: "WhatsApp", key: "whatsapp", placeholder: "Optional", type: "tel" },
            { label: "Notes", key: "notes", placeholder: "Additional details or address", type: "text" },
          ...
          <button type="submit" disabled={mut.isPending || checkingPhone}
            className="w-full py-3.5 rounded-xl font-bold text-white text-sm disabled:opacity-60 cursor-pointer"
            style={{ background: PRIMARY }}>
      ...
      <div className="relative w-full max-w-md rounded-t-3xl md:rounded-3xl shadow-2xl flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2.5 space-y-2.5">
          ...
          <p className="text-[10px] text-center pb-2" style={{ color: MUTED }}>
            Created {formatLocalDate(repair.createdAt)}
            {repair.createdAt !== repair.updatedAt && ` · Updated ${formatLocalDate(repair.updatedAt)}`}
          </p>
        </div>
      </div>
    </div>
...
function RepairForm({ onClose, existing }: { onClose: () => void; existing?: Repair }) {
  ...
  const { branchParam } = useBranchSelection();

  // ── Staff / technician list for the dropdown ─────────────────────────────
  const { data: staffData = [] } = useQuery<any[]>({
    queryKey: ["staff", isRepairFormFree, repairFormUser?.id, branchParam],
    queryFn: async () => {
  ...
  // Show technicians (or all active if no role filter matches)
  const techList = staffData.filter((s: any) =>
    s.isActive && s.role === "Technician"
  ).length > 0
    ? staffData.filter((s: any) => s.isActive && s.role === "Technician")
...
export default function Repairs() {
  ...
  const isManager = Boolean(repairPageUser?.isManager || repairPageUser?.role?.toLowerCase() === "manager");
  const isFreePlan = (repairPageUser?.plan === "Free" || !repairPageUser?.plan) && !repairPageUser?.isStaff && !isManager;

  const branchParam = activeBranch?.code === "MAIN" ? "MAIN" : (activeBranch?.id || "");

  const { data: repairs = [], isLoading } = useQuery<Repair[]>({
    queryKey: ["repairs", activeBranch?.id],
    queryFn: () => {
      if (isFreePlan && repairPageUser?.id) return Promise.resolve(localRepairs.getAll(repairPageUser.id));
      return fetch(`/api/repairs?branchId=${encodeURIComponent(branchParam)}`, { credentials: "include" }).then(r => r.json());
    },
  });
  ...
  const filtered = list.filter(r => {
    ...
    const phoneClean = r.customerPhone ? r.customerPhone.replace(/\D/g, "") : "";
    const phoneMatch = Boolean(
      r.customerPhone?.toLowerCase().includes(q) ||
      (qDigits.length >= 3 && phoneClean.includes(qDigits))
    );
    const brandMatch = r.phoneBrand?.toLowerCase().includes(q);
    const modelMatch = r.phoneModel?.toLowerCase().includes(q);
    const problemMatch = r.problem?.toLowerCase().includes(q);
    const notesMatch = r.notes?.toLowerCase().includes(q);
    const engineerMatch = r.engineer?.toLowerCase().includes(q);
    const idMatch = String(r.id) === q || `#${r.id}` === q;

    const matchSearch = nameMatch || phoneMatch || brandMatch || modelMatch || problemMatch || notesMatch || engineerMatch || idMatch;
        ...
        <div className="flex gap-2 items-center">
          <div className="flex-1 flex items-center gap-2 px-3.5 py-2.5 rounded-xl border"
            ...
            <input value={searchQ} onChange={e => setSearchQ(e.target.value)}
              placeholder="Search by name, phone, model, problem…"
