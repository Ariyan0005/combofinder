...
import {
  ArrowLeft, Plus, Tag, ChevronDown, FolderOpen, Edit3, Trash2, X, FolderPlus, Search, Building2
} from "lucide-react";
...
type Category = { id: number; name: string; description?: string; parentId?: number; branchId?: number | null; branchName?: string | null; };
...
function CategoryFormModal({
  mode, existing, parentCategories, onClose, isFree, userId, branchParam, branchName
}: {
...
export default function ManageCategories() {
  ...
  const { data: categories = [], isLoading } = useQuery<Category[]>({
    queryKey: ["inv-categories", activeBranch?.id, branchParam],
    queryFn: async () => {
      // ── Free plan: local storage ─────────────────────────────────────────
      if (isFree && userId) {
        return localCategories.getAll(userId, branchParam) as Category[];
      }
      // ── Pro plan: server ─────────────────────────────────────────────────
      const res = await fetch(`/api/inventory-categories?branchId=${encodeURIComponent(branchParam || "")}`, { credentials: "include" });
      if (!res.ok) return [];
      const d = await res.json();
      ...
      <div className="space-y-4 pb-8">
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-extrabold truncate">Manage Categories</h1>
            <p className="text-xs truncate" style={{ color: MUTED }}>
              {parentCats.length} categories • {activeBranch?.name || "Main Branch"}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-semibold text-sm text-white shadow-xs"
            style={{ background: PRIMARY }}>
            <FolderPlus className="w-4 h-4" /> Add Category
          </button>
          <button
            onClick={() => { setEditTarget(undefined); setFormMode("subcategory"); }}
            className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl font-semibold text-sm border shadow-xs"
        ...
        {!isLoading && categories.length === 0 && (
          <div className="text-center py-10 rounded-2xl border border-dashed p-6" style={{ borderColor: BORDER }}>
            <FolderOpen className="w-10 h-10 mx-auto mb-3" style={{ color: MUTED }} />
            <p className="font-semibold">No categories in {activeBranch?.name || "this branch"}</p>
            <p className="text-sm mt-1" style={{ color: MUTED }}>Add categories to organise your inventory for this branch</p>
          </div>
        )}
        ...
        {!isLoading && filteredParents.map(parent => (
          <div key={parent.id} className="rounded-2xl border overflow-hidden shadow-xs" style={{ borderColor: BORDER }}>
            {/* Parent row */}
