...
import { ArrowLeft, Smartphone, BatteryFull, Search } from "lucide-react";
...
interface DeviceCompat {
  id: number; batteryModelId: number; deviceName: string; notes?: string;
}

export default function BatteryModelPage() {
  const [, params] = useRoute("/battery-model/:id");
  const [, setLocation] = useLocation();
  const batteryModelId = Number(params?.id);
  const [search, setSearch] = useState("");

  const { data: batteryModel } = useQuery<BatteryModel>({
    queryKey: ["battery-model", batteryModelId],
    queryFn: () => fetch(`/api/battery-models/${batteryModelId}`, { credentials: "include" }).then(r => r.json()),
    enabled: !!batteryModelId,
  });

  const { data: devices = [], isLoading } = useQuery<DeviceCompat[]>({
    queryKey: ["battery-compat", batteryModelId],
    queryFn: () => fetch(`/api/battery-compat?batteryModelId=${batteryModelId}`, { credentials: "include" }).then(r => r.json()),
    enabled: !!batteryModelId,
  });
  ...
  const headerPal = batteryModel ? pal(batteryModel.modelNumber) : PALETTES[0];
    ...
    <ProtectedPage>
      <div className="space-y-4 pb-24">
        ...
        <div className="flex items-center gap-3 pt-1">
          <button onClick={() => batteryModel ? setLocation(`/battery-brand/${batteryModel.brandId}`) : history.back()}
            className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
            style={{ background: "hsl(var(--muted))", color: MUTED }}>
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-extrabold font-mono truncate">{batteryModel?.modelNumber ?? "Battery"}</h1>
            <p className="text-xs" style={{ color: MUTED }}>
              {batteryModel?.brandName}
              {batteryModel?.capacity && <> · {batteryModel.capacity}</>}
              {batteryModel?.voltage && <> · {batteryModel.voltage}</>}
            </p>
          </div>
        </div>
        ...
        {batteryModel && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-2xl"
            style={{ background: headerPal.bg }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: "white" }}>
              <BatteryFull className="w-5 h-5" style={{ color: headerPal.color }} />
            </div>
            <div>
              <p className="font-bold text-sm" style={{ color: headerPal.color }}>{batteryModel.modelNumber}</p>
              <div className="flex flex-wrap gap-2 mt-0.5">
                {batteryModel.capacity && (
                  <span className="text-[11px] font-medium" style={{ color: headerPal.color }}>{batteryModel.capacity}</span>
                )}
                {batteryModel.voltage && (
                  <span className="text-[11px] font-medium" style={{ color: headerPal.color }}>{batteryModel.voltage}</span>
                )}
              </div>
              {batteryModel.notes && (
                <p className="text-[11px] mt-0.5" style={{ color: headerPal.color }}>{batteryModel.notes}</p>
              )}
            </div>
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            {!search && <p className="text-sm mt-1" style={{ color: MUTED }}>Add compatible devices from Admin panel</p>}
          </div>
        ) : (
