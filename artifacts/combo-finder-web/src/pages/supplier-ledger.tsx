...
import {
  localSuppliers, localInventory, localCategories,
  localSupplierPurchases, localSupplierPayments,
} from "@/lib/local-store";
import { getLocalYMD } from "@/lib/date-utils";
import { BarcodeScanner, AddProductModal, type Item, type Supplier as InvSupplier, type Category } from "./inventory";
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input {...props}
      className={`w-full px-3.5 py-3 rounded-xl border text-sm outline-none focus:ring-2 ${props.className ?? ""}`}
...
function AddPurchaseModal({ supplierId, supplierName, onClose, isFree, userId }: {
  ...
  const { data: inventoryItems = [] } = useQuery<Item[]>({
    queryKey: ["inventory"],
    queryFn: async () => {
      if (isFree && userId) return localInventory.getAll(userId) as Item[];
      const r = await fetch("/api/inventory", { credentials: "include" });
      return r.json();
    },
  });

  const { data: suppliersList = [] } = useQuery<InvSupplier[]>({
    queryKey: ["suppliers"],
    queryFn: async () => {
      if (isFree && userId) return localSuppliers.getAll(userId) as InvSupplier[];
  ...
  const { data: categoriesList = [] } = useQuery<Category[]>({
    queryKey: ["categories"],
    queryFn: async () => {
      if (isFree && userId) return localCategories.getAll(userId) as Category[];
  ...
  const handleScanDetect = (code: string) => {
    ...
    if (found) {
      ...
      setError(`No product found matching "${code}". Try adding a new product or searching manually.`);
    }
  };
  ...
  const validLines = lines.filter(l => Number(l.qty) > 0 && Number(l.unitPrice) > 0 && l.item);

  const mut = useMutation({
    mutationFn: async () => {
      if (validLines.length === 0) throw new Error("Add at least one product with quantity and price");
    ...
    onSuccess: async () => {
      ...
      if (validLines.length > 0) qc.invalidateQueries({ queryKey: ["inventory"] });
      onClose();
    },
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
        ...
        <div className="relative w-full max-w-md rounded-t-3xl md:rounded-3xl shadow-2xl flex flex-col"
          style={{ background: CARD, maxHeight: "88vh" }}>
          <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b flex-shrink-0" style={{ borderColor: BORDER }}>
            <div>
              <h2 className="font-bold text-base">Purchase Invoice</h2>
              <p className="text-xs" style={{ color: MUTED }}>Supplier: {supplierName}</p>
            </div>
          ...
          <div className="px-5 py-5 overflow-y-auto flex-1">
            <form onSubmit={e => {
              ...
              {/* Products Section */}
              <div>
                <p className="text-xs font-semibold mb-1.5" style={{ color: MUTED }}>Products</p>

                {/* Universal Search Bar with Barcode Scanner inside & Add Product Button beside */}
                <div className="relative mb-3">
                  <div className="flex items-center gap-2">
                    {/* Search bar with barcode scanner icon inside */}
                    <div className="relative flex-1">
                      <Input
                        ...
                        className="pl-3.5 pr-16 text-sm"
                      />
                      {universalSearch ? (
                        <button
                          type="button"
                  ...
                  {showUniversalDrop && universalSearch.trim() && (() => {
                    ...
                    const hits = inventoryItems.filter(i =>
                ...
                {lines.length === 0 ? (
                  <div className="rounded-xl border border-dashed p-4 text-center text-xs" style={{ borderColor: BORDER, color: MUTED }}>
                    Search or scan product above to add to this invoice
                  </div>
                ) : (
                  <div className="flex flex-col gap-2">
                    {lines.map((line) => (
                      <div key={line._key} className="rounded-xl border p-3 space-y-2" style={{ borderColor: BORDER, background: BG }}>
                        ...
                        {Number(line.qty) > 0 && Number(line.unitPrice) > 0 && (
                          <p className="text-xs font-semibold text-right" style={{ color: PRIMARY }}>
