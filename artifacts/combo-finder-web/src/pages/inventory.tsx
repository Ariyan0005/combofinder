...
import { InventoryDemo } from "@/components/demo/inventory-demo";
...
function CategoryModal({ onClose, existing, mode, parentCategories, initialParentId, branchParam, branchName }: {
  onClose: () => void;
  existing?: Category;
...
function InventoryMain() {
  ...
  const isManager = Boolean(user?.isManager || user?.role?.toLowerCase() === "manager");
  const useLocalInventory = isInvFreePlan && !user?.isStaff && !isManager;

  const branchParam = activeBranch?.code === "MAIN" ? "MAIN" : (activeBranch?.id || "");

  const { data: items = [], isLoading } = useQuery<Item[]>({
    queryKey: ["inventory", activeBranch?.id],
    queryFn: async () => {
      if (useLocalInventory && user?.id) return localInventory.getAll(user.id);
      const d = await fetch(`/api/inventory?branchId=${encodeURIComponent(branchParam)}`, { credentials: "include" }).then(r => r.json());
  ...
  // Category tabs: "All" plus user categories
  ...
  const activeTab = catTabs.find(t => t.key === activeCategoryKey) || catTabs[0] || { key: "All", name: "All" };

  // Which parent is "active" (either directly selected, or is parent of selected sub)
  const activeParentKey = (() => {
    if (!activeTab?.parentId) return activeTab?.key ?? "All";
  ...
  const filtered = list.filter(item => {
    const matchCat = (() => {
      if (!activeTab || activeTab.key === "All") return true;
      if (activeTab.id != null) {
        // parent selected → include its children too; sub selected → exact match
        const childIds = (Array.isArray(categories) ? categories : []).filter(c => c.parentId === activeTab.id).map(c => c.id);
