...
import {
  ...
  Download, FileDown, ArrowLeft, UserCircle, CreditCard, Phone, Printer, MapPin, CalendarRange,
} from "lucide-react";
...
export default function Invoices() {
  ...
  const { data: sales = [], isLoading } = useQuery<Sale[]>({
    queryKey: ["sales", from, to, search, isPro, user?.id, activeBranch?.id],
    queryFn: () => {
      if (!isPro && user?.id) {
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    ...
                    d.shopName = shopName; d.shopAddress = shopAddress; d.shopPhone = user?.phone;
      ...
      <div className="space-y-3 pb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-2 rounded-xl text-xs font-bold border transition-all"
            style={isRangeFiltered
              ? { background: `${PRIMARY}15`, color: PRIMARY, borderColor: PRIMARY }
              : { borderColor: BORDER, color: MUTED }}>
            <CalendarRange className="w-3.5 h-3.5" />
            {isRangeFiltered && <X className="w-3 h-3 ml-0.5" onClick={e => { e.stopPropagation(); setFrom(""); setTo(""); setShowDateFilter(false); }} />}
          </button>
        ...
        {/* Compact date range — shown only when date icon is toggled */}
        {showDateFilter && (
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="date" value={from} onChange={e => setFrom(e.target.value)} max={to || undefined}
                className="w-full px-2.5 py-2 rounded-xl border text-xs outline-none" style={{ borderColor: BORDER, background: CARD }} />
            </div>
            <div>
