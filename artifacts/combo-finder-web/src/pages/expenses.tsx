...
const CURRENCY_SYMBOLS: Record<string, string> = {
  ...
  CHF: "Fr", HKD: "HK$", SGD: "S$", KRW: "₩", TWD: "NT$", BDT: "Tk",
  INR: "₹", PKR: "₨", NPR: "रू", LKR: "Rs", MVR: "Rf", BTN: "Nu",
  MYR: "RM", THB: "฿", IDR: "Rp", PHP: "₱", VND: "₫", MMK: "K",
  KHR: "៛", LAK: "₭", BND: "B$", MOP: "P", AED: "د.إ", SAR: "﷼",
  QAR: "﷼", KWD: "KD", BHD: "BD", OMR: "﷼", TRY: "₺", RUB: "₽",
  NGN: "₦", ZAR: "R", EGP: "E£", GHS: "₵", KES: "KSh", UGX: "USh",
  TZS: "TSh", ETB: "Br",
};
...
function formatMoney(amount: unknown, symbol: string): string {
  const value = Number(amount ?? 0);
  return `${symbol} ${Number.isFinite(value) ? value.toLocaleString(undefined, {
...
function ExpenseForm({ onClose, existing }: { onClose: () => void; existing?: Expense }) {
  ...
  const [form, setForm] = useState({
    ...
    date: existing?.date ?? getLocalYMD(),
  });
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative bg-card w-full max-w-md rounded-t-3xl md:rounded-3xl p-5 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
          ...
          <div>
            <label className="text-xs font-semibold block mb-1" style={{ color: "hsl(var(--muted-foreground))" }}>Amount ({sym})</label>
...
export default function Expenses() {
  ...
  const { data: expenses, isLoading } = useQuery<Expense[]>({
    queryKey: ["expenses", isPro, user?.id, activeBranch?.id],
    queryFn: () => {
      if (!isPro && user?.id) return Promise.resolve(localExpenses.getAll(user.id) as Expense[]);
      return fetch(`/api/expenses?branchId=${encodeURIComponent(branchParam)}`, { credentials: "include" }).then(r => r.json());
    },
    enabled: !!user?.id,
  });
  ...
  const filtered = list.filter(e => {
    ...
    const d = getLocalYMD(e.date ?? e.createdAt);
  ...
  function handleExportPdf() {
    const rows = filtered.map(e => {
      const d = getLocalYMD(e.date ?? e.createdAt);
      return `<tr><td>${escHtml(d)}</td><td>${escHtml(e.category)}</td><td>${escHtml(e.description)}</td><td style="text-align:right">${escHtml(sym)} ${Number(e.amount ?? 0).toFixed(2)}</td></tr>`;
    }).join("");
...
<table>
  ...
  <tbody>${rows}</tbody>
  <tfoot><tr><td colspan="3">Total</td><td style="text-align:right">${escHtml(sym)} ${totalAmount.toFixed(2)}</td></tr></tfoot>
</table>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-card rounded-2xl border border-border p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold mt-0.5">{formatMoney(totalAmount, sym)}</p>
          </div>
        </div>
            ...
            {filtered.map(e => (
              <div key={e.id} className="flex items-center gap-3 px-4 py-3.5">
                ...
                <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                  <span className="text-sm font-extrabold">{formatMoney(e.amount, sym)}</span>
