...
import {
  MapPin, Globe, Star, CheckCircle, Search, ChevronDown,
  MessageCircle, X, Send, Store,
} from "lucide-react";
...
export default function FindParts() {
  ...
  // Set SEO page title
  useEffect(() => {
    document.title = "Buy Mobile Spare Parts Near You | PosCert";
    return () => { document.title = "PosCert"; };
  }, []);
      ...
      >
        <div className="relative z-10 space-y-1">
          ...
          <div className="flex items-center gap-2 pt-1 flex-wrap">
            <button
              type="button"
              aria-label="Add Your Shop Listing"
              className="inline-flex items-center gap-1.5 bg-white text-xs font-bold px-3 py-1.5 rounded-full shadow-sm cursor-default"
              style={{ color: "hsl(var(--primary))" }}
            >
              Add Your Shop Listing
            </button>
            <a
              href="https://wa.me/96897043234"
              target="_blank"
              rel="noopener noreferrer"
               className="inline-flex items-center gap-1.5 bg-white text-xs font-bold px-3 py-1.5 rounded-full shadow-sm"
               style={{ color: "hsl(var(--primary))" }}
            >
              <MessageCircle className="w-3.5 h-3.5" />
              Contact Admin
      ...
      {detectedCountry && selectedCountry !== detectedCountry && (
        <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-muted/40 text-xs">
          <span style={{ color: "hsl(var(--muted-foreground))" }}>
            Detected location: <strong className="text-foreground">{detectedCountry}</strong>
          </span>
          <button
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center rounded-2xl border border-dashed"
          ...
          <p className="text-xs mt-1 mb-4" style={{ color: "hsl(var(--muted-foreground))" }}>
            {selectedCountry
              ...
              : "No spare parts suppliers listed yet."}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
