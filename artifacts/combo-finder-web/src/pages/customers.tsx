...
export default function CustomersAndSuppliers() {
  ...
  const isFreePlan = !user || user.plan === "Free";
  // Free owners keep party data locally; Pro users and staff use the API.
  const useLocalData = isFreePlan && !user?.isStaff;
  const userId = user?.id;
  ...
  const { data: rawCustomers = [], isLoading: isLoadingCustomers } = useQuery<Customer[]>({
    queryKey: ["customers", useLocalData ? "local" : "server", userId],
    queryFn: async () => {
  ...
  const { data: rawSuppliers = [], isLoading: isLoadingSuppliers } = useQuery<Supplier[]>({
    queryKey: ["suppliers", useLocalData ? "local" : "server", userId],
    queryFn: async () => {
      if (useLocalData && userId) {
  ...
  const { data: rawBalances = [] } = useQuery<SupplierBalance[]>({
    queryKey: ["suppliers-balances", useLocalData ? "local" : "server", userId],
    queryFn: async () => {
      if (useLocalData && userId) {
        return localSupplierPurchases.getAllBalances(userId);
      }
      const res = await fetch("/api/supplier-purchases/balances", { credentials: "include" });
      const d = await res.json();
      return Array.isArray(d) ? d : [];
    },
    enabled: !isGuest && !!user,
