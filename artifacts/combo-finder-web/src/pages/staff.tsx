...
function StaffForm({
  ...
  const canCreateLogin = user?.plan === "Pro";
  ...
  const [role,     setRole]     = useState<Role>(isGeneralStore ? "Staff" : (existing?.role ?? "Staff"));
  ...
  function handleSubmit(e: FormEvent) {
    ...
    setError("");
    if (canCreateLogin && !existing && !username.trim()) { setError("Username is required for Staff login"); return; }
    if (canCreateLogin && !existing && password.length < 8) { setError("Password must be at least 8 characters"); return; }
    saveMut.mutate({
      name: name.trim(), phone: phone.trim() || undefined, staffId: staffId.trim() || undefined,
      username: canCreateLogin ? username.trim() : undefined,
      password: canCreateLogin ? (password || undefined) : undefined,
      role, isActive, notes: notes.trim() || undefined,
    } as any);
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center md:items-center">
      ...
      <div className="relative w-full md:max-w-sm rounded-t-2xl md:rounded-2xl shadow-xl p-5 space-y-4"
        ...
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-base">{existing ? "Edit Member" : isGeneralStore ? "Add Staff" : "Add Staff / Technician"}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {/* Separate staff login is a Pro-only feature. */}
          {canCreateLogin ? (
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[11px] font-semibold mb-1 block" style={{ color: MUTED }}>Login username *</label>
                <input value={username} onChange={e => setUsername(e.target.value)} placeholder="e.g. rahul"
                  disabled={Boolean(existing)}
                  className="w-full px-3.5 py-2.5 rounded-xl border text-sm outline-none disabled:opacity-60"
                  style={{ borderColor: BORDER, background: BG }} />
              </div>
              <div>
                <label className="text-[11px] font-semibold mb-1 block" style={{ color: MUTED }}>
            ...
            <div className="rounded-xl px-3 py-2.5 text-xs" style={{ background: "hsl(var(--muted))", color: MUTED }}>
              Free plan staff members can be added without a separate login.
            </div>
          )}
...
export default function StaffPage() {
  ...
  const [filter,  setFilter]  = useState<"all" | Role>(isGeneralStore ? "Staff" : "all");
  ...
  const list = (Array.isArray(members) ? members : [])
    .filter(m => !isGeneralStore || m.role === "Staff");
      ...
      <div className="space-y-4 pb-8">
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-extrabold leading-tight">{isGeneralStore ? "Staff" : "Staff & Technician"}</h1>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-1.5 p-1 rounded-xl" style={{ background: "hsl(var(--muted))" }}>
            {(isGeneralStore ? (["Staff"] as const) : (["all", "Staff", "Technician", "Both"] as const)).map(f => (
