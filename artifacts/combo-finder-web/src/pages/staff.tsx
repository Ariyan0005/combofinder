...
import {
  ...
  Building2, KeyRound, Eye, EyeOff, User
} from "lucide-react";
...
const ROLE_CONFIG: Record<string, { label: string; color: string; bg: string; icon: typeof UserCheck }> = {
  ...
  Both:       { label: "Staff & Tech", color: "#F59E0B", bg: "#FFFBEB", icon: Users2      },
};

function RoleBadge({ role }: { role: string }) {
  const r = role === "Both" ? "Both" : role;
  const { label, color, bg, icon: Icon } = ROLE_CONFIG[r] ?? ROLE_CONFIG.Staff;
  return (
...
function StaffForm({
  onClose, existing, initialRole,
}: { onClose: () => void; existing?: StaffMember; initialRole?: Role }) {
  const { user } = useAuth();
  ...
  const [role,       setRole]       = useState<Role>(
    (existing?.role === "Manager" || existing?.role === "Technician" ? existing.role : "Staff") as Role
  );
  const [branchId,   setBranchId]   = useState(existing?.branchId   ?? "default");
...
export default function StaffPage() {
  ...
  const filtered = list.filter(m => {
    ...
    if (filter === "all") {
      ...
      matchRole = roleLower === "staff" || roleLower === "both";
    } else if (filterLower === "technician") {
      matchRole = roleLower === "technician" || roleLower === "both";
    } else {
  ...
  const totalStaff    = list.filter(m => ["staff", "both"].includes(String(m.role).toLowerCase())).length;
  const totalTech     = list.filter(m => ["technician", "both"].includes(String(m.role).toLowerCase())).length;
