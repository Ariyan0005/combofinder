...
import {
  ...
  Package, ChevronRight, ArrowLeft, Calendar, Building2,
} from "lucide-react";
...
export default function SalesReport() {
  ...
  const localSummary = useMemo(() => {
    ...
    return computeSalesSummary(user.id, range, customFrom, customTo, !isGeneralStore);
  }, [isFreePlan, user?.id, isGeneralStore, range, customFrom, customTo]);
  ...
  const apiParams = range === "custom"
    ? `range=custom&from=${customFrom}&to=${customTo}&tz=${userTZ}&branchId=${encodeURIComponent(branchParam)}`
    : `range=${range}&tz=${userTZ}&branchId=${encodeURIComponent(branchParam)}`;

  const { data: apiSummary, isLoading } = useQuery<any>({
    queryKey:    ["sales-summary", range, customFrom, customTo, activeBranch?.id],
  ...
  const repairRevenue   = isGeneralStore ? 0 : (s?.repairRevenue   ?? 0);
  const repairPartsCost = isGeneralStore ? 0 : (s?.repairPartsCost ?? 0);
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="rounded-2xl border p-4 space-y-3"
              ...
              {!isGeneralStore && repairRevenue > 0 && (
            ...
            <div className="space-y-2">
              {[
                ...
                ...(!isGeneralStore ? [{ label: "Repair Report", href: "/reports", icon: Wrench }] : []),
