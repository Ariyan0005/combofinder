...
import {
  ...
  BookMarked, Users2, Wrench, Store, BarChart2, MessageCircle, Building2, Sparkles
} from "lucide-react";
...
// Left: Staff & Technician, Party's, Invoices, Ledger, Expense
// Right: Find Spare Parts, Unlock Services, Technician Tool, Subscriptions, Settings
const MORE_ITEMS = [
  { label: "Manage Branch",          icon: Building2,    href: "/branches"         },
  { label: "Employees",              icon: Users2,      href: "/staff"           }, // Row 1 Left
  { label: "Find Spare Parts",       icon: Store,       href: "/find-parts"      }, // Row 1 Right
  { label: "Party's",                icon: Users,       href: "/customers"       }, // Row 2 Left
  { label: "Unlock Services",        icon: Unlock,      href: "/unlock-services" }, // Row 2 Right
  { label: "Invoices",               icon: FileText,    href: "/invoices"        }, // Row 3 Left
  { label: "Technician Tool",        icon: Search,      href: "/compatibility"   }, // Row 3 Right
...
const GENERAL_STORE_MORE_ITEMS = [
  { label: "Manage Branch",          icon: Building2,    href: "/branches"         },
  { label: "Employees",              icon: Users2,      href: "/staff"           },
  { label: "Party's",               icon: Users,       href: "/customers"       },
...
export default function MainLayout({ children }: { children: ReactNode }) {
  ...
  const { user, isGuest, guestMode, exitGuest, logout } = useAuth();
  ...
  const guestNav = isGeneralStore ? GENERAL_STORE_GUEST_NAV : GUEST_NAV;
  const bottomNav = isGuest ? guestNav : (isGeneralStore ? GENERAL_STORE_BOTTOM_NAV : BOTTOM_NAV);
  const moreItems = isGeneralStore ? GENERAL_STORE_MORE_ITEMS : MORE_ITEMS;
  const staffHidden = new Set(["/expenses", "/reports", "/sales-report", "/settings", "/subscription", "/staff", "/branches"]);
  const managerHidden = new Set(["/settings", "/subscription", "/staff", "/branches"]);
  const isPaidOwner = Boolean(isOwner && user?.plan && user.plan.toLowerCase() !== "free");
  const visibleMoreItems = moreItems.filter(item => {
    if (isStaff && staffHidden.has(item.href)) return false;
    if (isManager && managerHidden.has(item.href)) return false;
    if (item.href === "/branches" && !isPaidOwner) return false;
    return true;
  });
    ...
    <div className="flex min-h-screen w-full" style={{ background: "hsl(var(--background))" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-card border-b border-border sticky top-0 z-30">
          ...
          {/* Top action buttons: Demo button for Guest, Support for Premium, Avatar/Login */}
          <div className="flex items-center gap-2">
            {isGuest && (
              <button
                type="button"
                onClick={() => setGuestModalOpen(true)}
                className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-primary/30 text-primary bg-primary/5 hover:bg-primary/10 transition-colors mr-1 cursor-pointer"
              >
            ...
            {user && user.plan && user.plan.toLowerCase() !== "free" && (location === "/" || location === "/settings" || location.startsWith("/settings")) && (
              <a href={WHATSAPP_SUPPORT_URL} target="_blank" rel="noopener noreferrer">
                <button className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-lg text-white shadow-sm mr-1 cursor-pointer"
            ...
            {isGuest && (
              <Link href="/login">
                <button className="text-xs font-semibold px-3 py-1.5 rounded-lg cursor-pointer"
      ...
      {/* Guest Mode Business Type Switcher Modal */}
      {isGuest && (
