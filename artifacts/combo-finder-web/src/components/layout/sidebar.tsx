...
const NAV_ITEMS = [
  ...
  { label: "Parts Compatibility", icon: Search, href: "/compatibility" },
...
export default function Sidebar({ onClose }: { onClose: () => void }) {
  ...
  const isPaidOwner = Boolean(isOwner && user?.plan && ["pro", "premium"].some(plan => user.plan!.toLowerCase().startsWith(plan)));
  ...
  return (
    <aside className="w-64 flex flex-col h-screen sticky top-0"
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
        ...
        {user?.businessType !== "general_store" && isOwner && (
