...
export async function migrateBranches(): Promise<void> {
  try {
    ...
    await db.execute(sql.raw('ALTER TABLE "inventory_categories" ADD COLUMN IF NOT EXISTS "branch_id" integer'));
    await db.execute(sql.raw('ALTER TABLE "inventory_categories" ADD COLUMN IF NOT EXISTS "branch_name" text'));
    ...
    await db.execute(sql.raw('CREATE INDEX IF NOT EXISTS "inventory_categories_user_branch_idx" ON "inventory_categories" ("user_id", "branch_id")'));
