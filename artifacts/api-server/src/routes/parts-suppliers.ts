...
// ── GET /parts-suppliers  — public & admin listing ────────────────────────────
router.get("/", async (req, res) => {
  try {
    const { country, city, all } = req.query as Record<string, string | undefined>;

    const conditions: any[] = [];
    if (all !== "true" && all !== "1") {
      conditions.push(eq(partsSuppliersTable.isActive, true));
    }
...
// ── POST /parts-suppliers — admin-only supplier creation ───────────────────────
router.post("/", adminOnly, async (req: any, res) => {
  try {
    const { name, country, city, whatsapp, partTypes, website } = req.body;
    ...
    const [row] = await db.insert(partsSuppliersTable).values({
      ...
      partTypes: partTypes ? String(partTypes).trim() : null,
...
// Keep the admin collection route before /:id so "admin" is not parsed as an id.
...
router.post("/admin", adminOnly, async (req: any, res) => {
  try {
    ...
    const finalCountry = (country && String(country).trim()) ? String(country).trim() : "Bangladesh";
    const finalCity    = (city && String(city).trim())       ? String(city).trim()    : "General";
