...
router.get("/", async (req, res) => {
  try {
    ...
    const conditions = [eq(inventoryTable.userId, userId)];
    ...
    res.json(rows.map((row: any) => req.session?.userRole === "Staff"
...
router.get("/barcode/:code", async (req, res) => {
  try {
    ...
    const conditions = [eq(inventoryTable.userId, userId), barcodeFilter];
    const branchCond = getBranchCondition(req, inventoryTable.branchId);
    if (branchCond) conditions.push(branchCond);
