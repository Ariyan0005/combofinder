...
router.get("/", async (req, res) => {
  try {
    ...
    const mainBranch = rows.find(r => r.code === "MAIN");
    const mainBranchId = mainBranch?.id ? String(mainBranch.id) : null;

    // Get all staff counts accurately (aggregating NULL, 0, and MAIN branch id into Main branch)
    const staffRows = await db
      .select({ branchId: staffTable.branchId })
      .from(staffTable)
      .where(eq(staffTable.userId, userId));

    const staffMap = new Map<string, number>();
    let mainStaffCount = 0;
    for (const s of staffRows) {
      const bid = s.branchId ? String(s.branchId).trim().toLowerCase() : "";
      if (!bid || bid === "0" || bid === "null" || bid === "default" || bid === "default-main" || (mainBranchId && bid === mainBranchId.toLowerCase())) {
        mainStaffCount++;
      } else {
        staffMap.set(bid, (staffMap.get(bid) || 0) + 1);
      }
    }

    // Get all inventory counts accurately (aggregating NULL, 0, and MAIN branch id into Main branch)
    const invRows = await db
      .select({ branchId: inventoryTable.branchId })
      .from(inventoryTable)
    ...
    for (const i of invRows) {
      const bid = i.branchId ? Number(i.branchId) : 0;
      if (!bid || (mainBranch?.id && bid === mainBranch.id)) {
        mainInvCount++;
      } else {
    ...
    res.json(rows.map(row => {
      const isMain = row.code === "MAIN" || (mainBranch && row.id === mainBranch.id);
      const staffC = isMain ? mainStaffCount : (staffMap.get(String(row.id).toLowerCase()) || 0);
      const invC = isMain ? mainInvCount : (invMap.get(Number(row.id)) || 0);
