...
function isStaffSession(session: any): boolean {
  if (!session) return false;
  if (session.staffId) return true;
  const role = String(session.userRole ?? "").toLowerCase();
  return ["staff", "technician", "manager", "both"].includes(role);
}
...
// Branch management is a paid owner feature. Keep this check server-side so
// hiding the navigation for Free users is not the only protection.
export function requirePaidOwnerAuth(req: any, res: any, next: any) {
  if (!req.session?.authenticated || !req.session?.userId) {
    res.status(401).json({ error: "Unauthorized" }); return;
  }
  if (isStaffSession(req.session)) {
  ...
  const isPaidPlan = plan.startsWith("pro") || plan.startsWith("premium");
  if (!isPaidPlan) {
    res.status(403).json({ error: "Branch management is available on Pro or Premium plans only" }); return;
  }
  req.userId = req.session.userId;
  next();
}
...
router.use("/branches", requirePaidOwnerAuth, branchesRouter);
