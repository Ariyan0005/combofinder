...
router.get("/compatibilities", async (req, res): Promise<void> => {
  ...
  const baseQuery = db
    .select({
      ...
      isPublished: compatibilitiesTable.isPublished,
...
router.post("/compatibilities", async (req, res): Promise<void> => {
  const { modelId, name, comboType, partType, qualityGrade, notes, imageUrl, isPublished } = req.body;
...
router.put("/compatibilities/:id", async (req, res): Promise<void> => {
  ...
  const [compat] = await db.update(compatibilitiesTable)
    .set({
      ...
      ...(name && { name: name.trim() }),
      ...(comboType && { comboType }),
      ...(partType !== undefined && { partType: partType?.trim() || null }),
      qualityGrade: qualityGrade || null,
      notes: notes || null,
      imageUrl: imageUrl?.trim() || null,
