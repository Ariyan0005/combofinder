...
const resetLimiter    = rateLimit("reset",    10, 15 * 60 * 1000); // 10 attempts / 15 min (protects OTP brute-force)
...
const FROM_NAME = () => process.env["MAIL_FROM_NAME"] ?? "PosCert";
...
async function sendWelcomeEmail(name: string, email: string) {
  try {
    ...
    await transporter.sendMail({
      ...
      to: `"${name}" <${email}>`,
      subject: `Welcome to PosCert, ${name}!`,
      html: `<div style="font-family:Inter,Arial,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#ffffff;">
        <h2 style="color:#0080DB;margin-bottom:8px;">Welcome to PosCert!</h2>
        <p style="color:#374151;font-size:15px;line-height:1.6;">Hi <strong>${name}</strong>,</p>
        <p style="color:#374151;font-size:15px;line-height:1.6;">Your account has been created successfully. PosCert is a powerful POS & store management tool for your business.</p>
        <div style="margin:24px 0;">
          <a href="https://finder.iunlockd.com/login" style="background:#0080DB;color:#ffffff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;">Go to PosCert</a>
        </div>
        <p style="color:#6B7280;font-size:13px;margin-top:32px;">If you didn't create this account, you can ignore this email.</p>
      </div>`,
      text: `Welcome to PosCert, ${name}!\n\nYour account has been created. Login at: https://finder.iunlockd.com/login`,
    });
  } catch (err) {
...
async function sendPasswordResetEmail(name: string, email: string, code: string) {
  ...
  await transporter.sendMail({
    from: `"${FROM_NAME()}" <${FROM_EMAIL()}>`,
    to: `"${name}" <${email}>`,
    subject: "PosCert — Password Reset Code",
    html: `<div style="font-family:Inter,Arial,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#ffffff;">
...
async function sendVerificationEmail(name: string, email: string, code: string): Promise<boolean> {
  try {
    ...
    await transporter.sendMail({
      from: `"${FROM_NAME()}" <${FROM_EMAIL()}>`,
      to: `"${name}" <${email}>`,
      subject: "PosCert — Verify Your Email",
      html: `<div style="font-family:Inter,Arial,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#ffffff;">
        <h2 style="color:#0080DB;margin-bottom:8px;">Verify Your Email</h2>
        <p style="color:#374151;font-size:15px;line-height:1.6;">Hi <strong>${name}</strong>,</p>
        <p style="color:#374151;font-size:15px;line-height:1.6;">Use the code below to activate your PosCert account. It expires in <strong>15 minutes</strong>.</p>
        <div style="margin:24px 0;text-align:center;">
          <div style="display:inline-block;background:#F3F4F6;border-radius:12px;padding:18px 40px;">
            <span style="font-size:36px;font-weight:800;letter-spacing:8px;color:#0080DB;">${code}</span>
          </div>
      ...
      text: `PosCert — Verify Your Email\n\nYour verification code: ${code}\n\nThis code expires in 15 minutes.`,
    });
    return true;
  } catch (err) {
...
router.get("/auth/facebook/callback", async (req, res) => {
  ...
  if (error) {
    res.redirect("/login?error=" + encodeURIComponent((error_description as string) || "Facebook sign-in was cancelled or failed"));
    return;
  }
  if (!code || typeof code !== "string") {
    res.redirect("/login?error=" + encodeURIComponent("Invalid Facebook authorization code"));
    return;
  }
  ...
  try {
    ...
    const tokenUrl = `https://graph.facebook.com/v19.0/oauth/access_token?client_id=${appId}&redirect_uri=${encodeURIComponent(callbackUrl)}&client_secret=${appSecret}&code=${encodeURIComponent(code)}`;
    const tokenRes = await fetch(tokenUrl);
    const tokenData = (await tokenRes.json()) as { access_token?: string; error?: { message?: string } };

    if (!tokenData.access_token) {
      req.log.error({ tokenData }, "Facebook token exchange failed");
      res.redirect("/login?error=" + encodeURIComponent(tokenData.error?.message || "Facebook token exchange failed"));
      return;
    }
    ...
    const fbUser = (await userRes.json()) as { id?: string; name?: string; email?: string; picture?: { data?: { url?: string } } };

    let userEmail = fbUser.email;
    if (!userEmail || !userEmail.includes("@")) {
      userEmail = `fb_${fbUser.id}@facebook.user`;
    }

    const normalizedEmail = userEmail.toLowerCase().trim();
    const displayName = (fbUser.name && fbUser.name.trim()) || `Facebook User`;
      ...
      try {
        const [newUser] = await db
          ...
          .values({
            ...
            isApproved: true,
            currency: "USD",
            shopLogo: fbUser.picture?.data?.url || null,
          })
          .returning();
        user = newUser;
      } catch (insertErr) {
        req.log.error({ insertErr }, "Failed to create new user from Facebook");
        res.redirect("/login?error=" + encodeURIComponent("Could not complete Facebook registration. Please try email sign-up."));
        return;
      }
    }
    ...
    res.redirect("/login?error=" + encodeURIComponent(err.message || "Facebook Authentication Failed"));
  }
});

// GitHub OAuth Redirect (Initiate)
...
router.get("/auth/github/callback", async (req, res) => {
  ...
  if (!code || typeof code !== "string") {
    res.redirect("/login?error=" + encodeURIComponent("Invalid GitHub authorization code"));
    return;
  }
  ...
  if (!clientId || !clientSecret) {
    res.redirect("/login?error=" + encodeURIComponent("GitHub Client ID or Secret is missing"));
    return;
  }
  ...
  try {
    ...
    if (!tokenData.access_token) {
      ...
      res.redirect("/login?error=" + encodeURIComponent(tokenData.error_description || "GitHub authentication failed"));
      return;
    }
    ...
    if (!userEmail || !userEmail.includes("@")) {
      res.redirect("/login?error=" + encodeURIComponent("Could not retrieve a valid email from your GitHub account"));
      return;
    }

    const normalizedEmail = userEmail.toLowerCase().trim();
    const displayName = (ghUser.name && ghUser.name.trim()) || ghUser.login || normalizedEmail.split("@")[0];
      ...
      try {
        const [newUser] = await db
          ...
          .values({
            ...
            isApproved: true,
            currency: "USD",
            shopLogo: ghUser.avatar_url || null,
          })
          .returning();
        user = newUser;
      } catch (insertErr: any) {
    ...
    res.redirect("/login?error=" + encodeURIComponent("Failed to complete GitHub sign-in. Please try again."));
  }
});

// Google Sign-In & Registration handler
router.post("/auth/google", async (req, res) => {
  const { email, name, picture, googleId } = req.body as {
    email?: string;
    name?: string;
    picture?: string;
    googleId?: string;
  ...
  try {
    ...
    if (existingUsers.length > 0) {
      user = existingUsers[0];
      // If user was previously inactive or unapproved, activate them now
      if (!user.isActive || !user.isApproved) {
        await db
          .update(usersTable)
          .set({ isActive: true, isApproved: true, updatedAt: new Date() })
      ...
      try {
        const [newUser] = await db
          ...
          .values({
            ...
            isApproved: true,
            currency: "USD",
            shopLogo: picture || null,
          })
          .returning();
        user = newUser;
      } catch (insertErr: any) {
        // Fallback for older database schemas missing optional columns
        const [newUser] = await db
          .insert(usersTable)
          .values({
            name: displayName,
...
router.post("/auth/login", loginLimiter, async (req, res) => {
  ...
  // Staff accounts use the same login screen, but retain the owner's data scope.
  // They are checked before owner accounts so a Staff username never needs a fake email.
  try {
    const staffRows = await db.select().from(staffTable)
      .where(eq(staffTable.username, identifier.toLowerCase())).limit(1);
    const staff = staffRows[0];
    if (staff?.passwordHash) {
      if (!staff.isActive || !verifyPassword(password, staff.passwordHash)) {
        res.status(401).json({ error: "Invalid username or password" }); return;
      }
      const [owner] = await db.select().from(usersTable).where(eq(usersTable.id, staff.userId)).limit(1);
      ...
      if (staffRole.toLowerCase() === "technician") {
        res.status(403).json({ error: "Technician accounts do not have portal login access" }); return;
      }
      const isManager = staffRole.toLowerCase() === "manager";
      (req.session as any).authenticated = true;
      (req.session as any).userId = owner.id;
      (req.session as any).userName = staff.name;
      (req.session as any).userRole = staffRole;
      (req.session as any).staffId = staff.id;
      (req.session as any).branchId = staff.branchId ?? null;
      (req.session as any).branchName = staff.branchName ?? null;
      (req.session as any).ownerId = owner.id;
      (req.session as any).userPlan = owner.subscriptionPlan ?? "Free";
      (req.session as any).userCurrency = owner.currency ?? "USD";
      (req.session as any).userBusinessType = owner.businessType ?? "mobile_repair";
      res.json({ success: true, user: {
        id: owner.id, name: staff.name, email: "", phone: staff.phone ?? "",
        role: staffRole, plan: owner.subscriptionPlan ?? "Free",
        currency: owner.currency ?? "USD", shopName: owner.shopName ?? "",
        shopAddress: owner.shopAddress ?? "", shopLogo: owner.shopLogo ?? null,
        businessType: owner.businessType ?? "mobile_repair",
        isStaff: !isManager,
        isManager,
        staffId: staff.id,
        branchId: staff.branchId ?? undefined,
        branchName: staff.branchName ?? undefined,
      } });
      return;
    }
  } catch (err) {
    req.log.warn({ err }, "Staff login lookup failed; continuing with owner login");
  }
  ...
  try {
    ...
    (req.session as any).userCurrency = user.currency ?? "USD";
    (req.session as any).userBusinessType = user.businessType ?? "mobile_repair";
    res.json({ success: true, user: { id: user.id, name: user.name, email: user.email, phone: user.phone ?? "", role: user.accountType, plan: user.subscriptionPlan ?? "Free", currency: user.currency ?? "USD", shopName: user.shopName ?? "", shopAddress: user.shopAddress ?? "", shopLogo: user.shopLogo ?? null, businessType: user.businessType ?? "mobile_repair" } });
  } catch (err) {
  ...
  if ((req.session as any).authenticated) {
    ...
    res.json({
      ...
      user: {
        ...
        isStaff: (req.session as any).userRole === "Staff" || (req.session as any).userRole === "Technician" || (Boolean((req.session as any).staffId) && (req.session as any).userRole !== "Manager"),
        isManager: (req.session as any).userRole === "Manager",
        staffId: (req.session as any).staffId,
        branchId: (req.session as any).branchId ?? undefined,
        branchName: (req.session as any).branchName ?? undefined,
      },
    });
  } else {
...
router.put("/auth/settings", async (req, res) => {
  ...
  const userId = (req.session as any).userId;
  if (!userId) { res.status(400).json({ error: "No user ID in session" }); return; }
  const { currency, shopName, shopAddress, shopLogo, phone, businessType } = req.body as {
    currency?: string; shopName?: string; shopAddress?: string; shopLogo?: string; phone?: string; businessType?: string;
  };
  try {
    ...
    if (businessType !== undefined && (businessType === "mobile_repair" || businessType === "general_store")) {
      updates.businessType = businessType;
      (req.session as any).userBusinessType = businessType;
    }
