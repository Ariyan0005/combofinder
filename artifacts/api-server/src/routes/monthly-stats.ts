...
router.get("/monthly-stats", async (req: any, res) => {
  try {
    ...
    const repConds: any[] = [eq(repairsTable.userId, userId)];
    const branchCondR = getBranchCondition(req, repairsTable.branchId);
    if (branchCondR) repConds.push(branchCondR);

    const repairs = await db.select().from(repairsTable)
      .where(and(...repConds))
      .orderBy(repairsTable.createdAt);

    const expConds: any[] = [eq(expensesTable.userId, userId)];
    const branchCondE = getBranchCondition(req, expensesTable.branchId);
    if (branchCondE) expConds.push(branchCondE);
