...
// GET /api/inventory-categories — return user's categories for branch
router.get("/", async (req, res) => {
  try {
    const userId = getUid(req, res); if (!userId) return;
    const userFilter = eq(inventoryCategoriesTable.userId, userId);
    const branchCond = getBranchCondition(req, inventoryCategoriesTable.branchId);
    const filter = branchCond ? and(userFilter, branchCond) : userFilter;
...
router.post("/", async (req, res) => {
  try {
    ...
    const { branchId: savedBranchId, branchName: savedBranchName } = extractBranchSaveData(req, req.body);
...
router.put("/:id", async (req, res) => {
  try {
    const userId = getUid(req, res); if (!userId) return;
    const { name, description, icon, color, parentId, branchId, branchName } = req.body;
    ...
    if (branchName !== undefined) updateData.branchName = branchName ? String(branchName) : null;
