...
async function deductPartsOnCreation(
  ...
  for (const part of parts) {
    ...
    if (branchCond) invConds.push(branchCond);
...
router.get("/", async (req, res) => {
  try {
    ...
    const combined = [userFilter, customerFilter, branchCond].filter(Boolean) as any[];
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const userFilter = eq(repairsTable.userId, userId);
    const branchCond = getBranchCondition(req, repairsTable.branchId);
    const baseConditions = [userFilter, branchCond].filter(Boolean) as any[];
    ...
    const [total] = await db.select({ count: sql<number>`cast(count(*) as int)` })
      .from(repairsTable).where(and(...baseConditions));
