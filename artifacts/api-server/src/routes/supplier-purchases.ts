...
router.get("/", async (req, res) => {
  try {
    ...
    const branchCond = getBranchCondition(req, supplierPurchasesTable.branchId);

    const conditions: any[] = [eq(supplierPurchasesTable.userId, userId)];
    if (supplierId) conditions.push(eq(supplierPurchasesTable.supplierId, supplierId));
    if (branchCond) conditions.push(branchCond);

    const rows = await db.select().from(supplierPurchasesTable)
...
router.get("/balance", async (req, res) => {
  try {
    ...
    }).from(supplierPurchasesTable).where(and(...conditions));

    res.json({
      totalPurchased: Number(totals?.totalPurchased ?? 0),
...
router.get("/payments", async (req, res) => {
  try {
    const userId = getUid(req, res); if (!userId) return;
    const supplierId = req.query.supplierId ? Number(req.query.supplierId) : null;
    const branchCond = getBranchCondition(req, supplierPaymentsTable.branchId);

    const conditions: any[] = [eq(supplierPaymentsTable.userId, userId)];
    if (supplierId) conditions.push(eq(supplierPaymentsTable.supplierId, supplierId));
    if (branchCond) conditions.push(branchCond);
