        ...
        <div className="space-y-4">
          {searching ? (
            ...
            <>
              ...
              {/* IC number results */}
              {allIcModels.length > 0 && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide mb-2" style={{ color: MUTED }}>IC Numbers</p>
                  <div className="rounded-2xl border divide-y overflow-hidden" style={{ borderColor: BORDER, background: CARD }}>
                    {allIcModels.map((m: any) => (
                      <Link key={m.id} href={`/ic-model/${m.id}`}>
                        <div onClick={() => handleSearchSelect(m.icNumber)}
                          className="flex items-center justify-between px-4 py-3 cursor-pointer transition-colors hover:bg-muted/30">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                              style={{ background: "hsl(var(--muted))" }}>
                              <Cpu className="w-4 h-4" style={{ color: MUTED }} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold truncate">{m.icNumber}</p>
                              <p className="text-[10px] truncate" style={{ color: MUTED }}>
                                {m.brandName}{m.description ? ` · ${m.description}` : ""}{m.package ? ` · ${m.package}` : ""}
                              </p>
                            </div>
                          </div>
                          <ChevronRight className="w-4 h-4 flex-shrink-0 ml-2" style={{ color: MUTED }} />
              ...
              {/* Battery model results */}
              {allBatteryModels.length > 0 && (
                <div>
                      ...
                      <Link key={m.id} href={`/battery-model/${m.id}`}>
                        <div onClick={() => handleSearchSelect(m.modelNumber)}
                          ...
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                              style={{ background: "hsl(var(--muted))" }}>
                              <BatteryFull className="w-4 h-4" style={{ color: MUTED }} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold truncate">{m.modelNumber}</p>
                              <p className="text-[10px] truncate" style={{ color: MUTED }}>
                                {m.brandName}{m.capacity ? ` · ${m.capacity}` : ""}{m.voltage ? ` · ${m.voltage}` : ""}
                              </p>
                            </div>
                          </div>
                          <ChevronRight className="w-4 h-4 flex-shrink-0 ml-2" style={{ color: MUTED }} />
