...
router.post("/auth/register", registerLimiter, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // If an older VPS schema is missing an optional column, retry with the
      // legacy-safe subset. Do not include business_type here: if it is
      // missing, the selected General Store UI must never be silently changed
      // into Mobile Repair.
